...
namespace AssetStudioGUI
{
    partial class AssetStudioGUIForm : Form
    {
        ...
        private async void CompreTwo_Click(object sender, EventArgs e)
        {
            openFileDialog1.InitialDirectory = openDirectoryBackup;
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                openDirectoryBackup = Path.GetDirectoryName(openFileDialog1.FileNames[0]);
                var compareMgr = new AssetsManager();

                compareMgr.SpecifyUnityVersion = specifyUnityVersion.Text;
                await Task.Run(() => compareMgr.LoadFiles(openFileDialog1.FileNames));

                var ret = assetsManager.CompreToOther(compareMgr);
                ...
                List<AssetItem> otherAssets = new List<AssetItem>();
                parseAssetItems(compareMgr, objectAssetItemDic, ref productName, containers, ref i, otherAssets, () => { Progress.Report(++i, objectCount); });

                var dictOldAssets = new Dictionary<string, List<AssetItem>>();

                foreach(var asset in exportableAssets)
                {
                    List<AssetItem> listItems;

                    if (dictOldAssets.TryGetValue(asset.Text, out listItems))
                    {
                        listItems.Add(asset);
                    }
                    else
                    {
                        listItems = new List<AssetItem>();
                        listItems.Add(asset);
                        dictOldAssets.Add(asset.Text, listItems);
                    }
                ...
                var dictNewAssets = new Dictionary<string, List<AssetItem>>();
                ...
                foreach (var asset in otherAssets)
                {
                    ...
                    if (dictNewAssets.TryGetValue(asset.Text, out listItems))
                    {
                        listItems.Add(asset);
                    }
                    else
                    {
                        listItems = new List<AssetItem>();
                        listItems.Add(asset);
                        dictNewAssets.Add(asset.Text, listItems);
                    }

                    List<AssetItem> oldAssets;

                    if (dictOldAssets.TryGetValue(key, out oldAssets))
                    {
                        var findItem = oldAssets.Find(x => (x.Type == asset.Type && x.FullSize == asset.FullSize));

                        if (findItem == null)
                        {
                            log.AppendLine($"{asset.Text}_{asset.Type}_{asset.m_PathID}_{asset.FullSize}");
                        }
                    }
                    else
                    {
                        log.AppendLine($"new more item {asset.Text}_{asset.Type}_{asset.m_PathID}_{asset.FullSize}");
                    }
                }

                foreach(var asset in exportableAssets)
                {
                    List<AssetItem> newAssets;
                    var key = $"{asset.Text}";
                    if (dictNewAssets.TryGetValue(key, out newAssets))
                    {
                        var findItem = newAssets.Find(x => (x.Type == asset.Type && x.FullSize == asset.FullSize));
                    ...
                    {
                        log.AppendLine($"old more item {asset.Text}_{asset.Type}_{asset.m_PathID}_{asset.FullSize}");
                    }
                }
